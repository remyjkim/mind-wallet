...
const tapPath = resolve(args.tapPath ?? process.env['MINDPASS_TAP_PATH'] ?? '');

if (!tapPath) {
  throw new Error('Provide --tap-path or set MINDPASS_TAP_PATH');
}
...
writeFileSync(join(formulaDir, 'mindpass.rb'), formula, 'utf8');
